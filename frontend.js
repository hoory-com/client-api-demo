$(function () {
    ...
    hoory.inboxIdentifier = "bXKr2F2asDjztoVipLF9Lt8v";
    hoory.hooryAPIUrl = "http://localhost:3000/public/api/v1/"
    ...
    connection.onopen = function () {
        ...
        // first we want users to subscribe to the hoory webhooks
        connection.send(JSON.stringify({command:"subscribe", identifier: "{\"channel\":\"RoomChannel\",\"pubsub_token\":\""+hoory.contactPubsubToken+"\"}" }));
    ...
    function setUpContact() {
      if(getCookie('contactIdentifier')){
        hoory.contactIdentifier = getCookie('contactIdentifier');
        hoory.contactPubsubToken = getCookie('contactPubsubToken')
      }else{
        xhttp.open("POST", hoory.hooryAPIUrl + "inboxes/"+hoory.inboxIdentifier+"/contacts");
        xhttp.send();
    ...
    function setUpConversation() {
      if(getCookie('contactConverstion')){
        hoory.contactConverstion = getCookie('contactConverstion');
      }else{
        xhttp.open("POST", hoory.hooryAPIUrl + "inboxes/"+hoory.inboxIdentifier+"/contacts/"+hoory.contactIdentifier+"/conversations", false);
        xhttp.send();
    ...
    function sendMessage(msg){
      xhttp.open("POST", hoory.hooryAPIUrl + "inboxes/"+hoory.inboxIdentifier+"/contacts/"+hoory.contactIdentifier+"/conversations/"+hoory.contactConverstion+"/messages", false);
